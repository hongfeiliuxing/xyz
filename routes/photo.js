...
router.post('/add', function (req, res) {
    ...
    //fileName='as';
    data.url = fileName;
    //return res.send({success: false, message: 'file 不能为空'});
...
var sqlPhotoWall2 = 'select a.id as photo_id,sex,user_info.user_id,user_info.nickname,user_info.icon,url,a.good as count from photo a,user_info where a.state != 2 and a.activity = 1 and{0} user_info.user_id = a.user_id and 3>(select count(*) from photo where user_id=a.user_id and id>a.id) order by rand() limit 0,10';
